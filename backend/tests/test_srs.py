...
class TestReviewCardSuccess:
    ...
    def test_interval_minimum_is_exactly_1_not_2(self):
        """L'intervalle minimum est 1 (pas 2) — tue la mutation max(2, ...)."""
        # Carte avec interval=1, reps=2, ef=EF_MIN=1.3 (très bas).
        # round(1 * 1.3) = round(1.3) = 1. max(1, 1) = 1 (pas max(2, 1) = 2).
        card = SRSCard(
            id="test", fen="f", solution="s",
            ef=EF_MIN, interval=1, reps=2,
            due=date.today().isoformat(),
        )
        result = review_card(card, 3)  # reps=3, interval = max(1, round(1 * 1.3)) = 1
        # delta(q=3) = 0.1 - 2*(0.08+2*0.02) = -0.14 → ef=max(1.3, 1.3-0.14)=1.3
        # interval = max(1, round(1 * 1.3)) = max(1, 1) = 1
        assert result.interval == 1  # DOIT être 1, pas 2

    def test_ef_rounded_to_4_decimal_places(self):
        """L'EF est arrondi à 4 décimales exactement, pas 5 — tue la mutation round(...,5)."""
        # On utilise une carte avec ef=1.3 et quality=5 plusieurs fois pour obtenir
        # un EF qui diffère à la 5e décimale.
        # delta(q=5) = 0.1. ef = 1.3 + 0.1 = 1.4 (exact — testons une chaîne de reviews).
        # La vraie différence : on vérifie que round(x, 4) != round(x, 5) pour au moins
        # une valeur produite par la chaîne de calcul.
        # Truc : ef=2.5, quality=3 → delta=-0.14 → ef=2.3600000000000003 en float.
        # round(2.3600000000000003, 4) = 2.36
        # round(2.3600000000000003, 5) = 2.36
        # Essayons avec quality=3 et ef=1.34001 → delta=-0.14 → 1.20001
        # round(1.20001, 4) = 1.2 mais la valeur >= EF_MIN → clampé à 1.3
        # En fait, le moyen le plus sûr : vérifier que le nb de décimales ne dépasse pas 4.
        card = SRSCard(
            id="test", fen="f", solution="s",
            ef=2.5, interval=1, reps=0,
            due=date.today().isoformat(),
        )
        result = review_card(card, 3)  # ef = round(2.36..., 4) = 2.36
        # Si c'était round(..., 5), on aurait potentiellement plus de décimales.
        ef_str = str(result.ef)
        decimal_part = ef_str.split(".")[-1] if "." in ef_str else ""
        assert len(decimal_part) <= 4, f"EF a trop de décimales : {result.ef}"

        # Test direct : la valeur est bien la même qu'un round à 4.
        assert result.ef == round(result.ef, 4)

    def test_ef_round_4_not_5_decimal_places(self):
        """round(new_ef, 4) doit couper à 4 décimales, pas 5 — mutation-killer direct.

        ef=1.30001 + delta(q=5)=0.1 → 1.40001...
        round(1.40001, 4) = 1.4  (≠ 1.40001 qui serait le résultat de round(..., 5)).
        """
        card = SRSCard(
            id="precision_test", fen="f", solution="s",
            ef=1.30001, interval=1, reps=0,
            due=date.today().isoformat(),
        )
        result = review_card(card, 5)  # delta = +0.1 → new_ef = 1.40001...
        # round(1.40001..., 4) = 1.4 ; round(1.40001..., 5) = 1.40001
        assert result.ef == 1.4  # prouve que c'est bien round(..., 4) et pas round(..., 5)
