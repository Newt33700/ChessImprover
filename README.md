...
- **API Puzzle Lichess en source primaire (EPIC 34, `domain/lichess_puzzles.py` + `infrastructure/lichess_client.py`)** : `GET /tactics/next` essaie d'abord Lichess (`angle_for_theme` mappe `mate_in_1`/`mate_in_2`/`hanging_piece` → `mateIn1`/`mateIn2`/`hangingPiece` Lichess) — des millions de puzzles déjà vérifiés, sans les limites d'un petit seed statique. `parse_puzzle_payload` traduit la réponse (`game.pgn` rejoué jusqu'à `puzzle.initialPly` via python-chess, `puzzle.solution[0]` auto-joué pour obtenir le FEN de départ, le reste devient la séquence à résoudre) ; toute panne (réseau, timeout, forme inattendue) est loggée puis déclenche un repli **immédiat** sur le seed local — même politique que Chess.com (§4.2). Les puzzles récupérés sont insérés dans le même registre que le seed (`db_client.add_lichess_tactical_problem`, cache borné à 200 entrées en FIFO).
...
| `test_lichess_puzzles.py` | `angle_for_theme` (mapping des 3 thèmes, `None`, thème inconnu), `replay_pgn_to_ply` (rejeu exact, 0 coup, trop de coups, PGN illisible/vide), `parse_puzzle_payload` (payload bien formé, catégorie par défaut "aleatoire", champs manquants/malformés, coup de mise en place illégal, rating non entier — jamais d'exception) | EPIC 34 |
