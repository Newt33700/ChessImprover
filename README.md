...
### 11.3 Puzzles externes — ✅ Traité par EPIC 34

`GET /tactics/next` interroge désormais l'API Puzzle Lichess (`endpoint`, pas de CSV statique) en source primaire, par thème, avec repli sur le seed local (§4.8, EPIC 34). **Reste pour aller plus loin** : le CSV complet de la base Lichess (`lichess_db_puzzle.csv`, des millions de puzzles avec métadonnées riches — thèmes multiples, popularité) permettrait un import batch pour un mode hors-ligne ou un filtrage plus fin (ex. combiner plusieurs thèmes), non fait ici (l'endpoint `/api/puzzle/next` suffit au besoin exprimé : varier les exercices par thème).
