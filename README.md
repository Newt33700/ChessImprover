...
| Échiquier | Chessground v9.2.1 (échiquier principal, EPIC 37 ; bundlé IIFE avec esbuild) + chessboard.js v1.0.0 (échiquiers de problèmes ad-hoc) — vendorisés localement, `assets/js/`, EPIC 13/37 |
...
│   │   ├── board_manager.js            # Échiquier principal (Chessground, EPIC 37), Worker Stockfish, modes Review/Ghost/Sandbox
...
│   │   ├── js/                         # jquery-3.7.1.min.js, chess-0.10.3.js, chessboard-1.0.0.min.js, chessground.min.js (EPIC 37, bundlé IIFE esbuild), chart-4.4.0.umd.js
│   │   ├── css/                        # chessboard-1.0.0.min.css, chessground.base.css (EPIC 37), fonts.css (Google Fonts vendorisées)
...
│   ├── requirements.txt                # fastapi, uvicorn, httpx, pydantic, python-chess, bcrypt, psycopg, zstandard (EPIC 37)
...
│       │   ├── lichess_puzzles.py      # EPIC 34 : parsing API Puzzle Lichess ; EPIC 37 : resolve_random_puzzles (fallback Elo ±100)
│       │   └── puzzles_models.py       # EPIC 37 : schémas catalogue lichess_puzzles (LichessTheme, PuzzleQueryParams, PuzzleResponse)
│       ├── infrastructure/
...
│       │   ├── tactics.py              # GET /tactics/next, /tactics/custom (EPIC 11), /tactics/random (EPIC 37), POST /tactics/attempt (US 8.1)
...
│       │   ├── test_puzzles_service.py # EPIC 37 : resolve_random_puzzles (standard + fallback ±100), schémas PuzzleQueryParams/PuzzleResponse, route GET /tactics/random
│       │   └── test_ingest_lichess_puzzles.py # EPIC 37 : parse_puzzle_row (mapping CSV → lichess_puzzles, thèmes/tags espace → TEXT[])
...
│       └── 20260706000000_puzzles_service.sql # Table lichess_puzzles (catalogue Lichess local, sans RLS — lecture serveur seule), index B-Tree rating + GIN themes (EPIC 37)
│
└── .github/
    └── workflows/
...
### 3.13 ter EPIC 26 — Séparation des pages & fiabilisation des échiquiers (UX client)

**Fichiers :** `index.html` (`#exercise-col`), `js/app.js` (section Page Exercice SRS + `_createProblemBoard`), `js/board_manager.js` (mode exercice supprimé), `js/analysis_feedback.js` (`isExerciseMoveCorrect`)
...
- **Flèches (US 31.4)** : légende sous le plateau, flip géré, nettoyées hors Review. **EPIC 37** : dessinées par Chessground (`AnalysisFeedback.drawFeedback`/`cg.setShapes()`) — rouge (coup joué/erreur) + verte (suggestion moteur) — plus par l'overlay SVG maison décrit ici à l'origine (§3.13 undecies).
...
- **Câblage** : `_createProblemBoard` (fabrique commune, EPIC 26) appelle `TapMove.attach` dès qu'un `getChess` est fourni — ajouté aux 6 échiquiers de problèmes (Exercice SRS, Coach Tactique, Cimetière des Erreurs, Entraîneur d'Ouvertures, Technique de Mat, Tactical Sprint). **EPIC 37** : le board principal (Review/Fantôme/Bac à sable) est passé à Chessground, dont le clic-clic est **natif** (`movable.dests`) — `board_manager.js` n'attache plus `TapMove` (§3.13 undecies) ; les 6 échiquiers de problèmes ci-dessus restent inchangés (chessboard.js + `tap_move.js`).

---

### 3.13 decies EPIC 36 — Bugfix tap-to-move (vrai clic jamais fonctionnel) + UX Coach Tactique (trait, chronomètre, disposition large écran)

**Fichiers :** `js/tap_move.js`, `js/app.js` (Coach Tactique), `index.html` (`#tactics-col`), `css/style.css`

- **Bug corrigé — le clic-clic (§3.13 novies) ne fonctionnait en réalité jamais avec une vraie souris/trackpad**, seulement via `element.click()` synthétique (les tests) : chessboard.js (`draggable:true`) détache la pièce cliquée de sa case dès le `mousedown` — même sans aucun mouvement — pour la repositionner en `position:absolute` à même le `<body>` (mécanisme interne de glisser-déposer). L'événement `click` final se déclenche donc sur ce nœud flottant, hors du conteneur de l'échiquier, jamais vu par l'écouteur délégué. `TapMove.attach` écoute désormais `pointerdown` (conteneur) + `pointerup` (`document`, seul point garanti de recevoir l'événement) et résout la case tapée par **coordonnées** (`document.elementsFromPoint`), jamais via `event.target` ; un déplacement > 10px entre les deux est traité comme un vrai glisser-déposer (laissé tel quel à chessboard.js) pour ne jamais dupliquer un coup.
- **Coach Tactique — indicateur de trait** : « ● Trait aux Blancs/Noirs » au-dessus de l'échiquier (`#tactics-turn-indicator`), mis à jour à chaque problème et après la réplique adverse auto-jouée (mat en 2).
- **Coach Tactique — chronomètre visible** : badge `#tactics-timer-badge` (`⏱ m:ss`), démarré au chargement du problème, arrêté à la résolution ou en quittant la vue (`_setActiveView`, garde-fou centralisé quel que soit le chemin de sortie).
- **Coach Tactique — disposition large écran** : `#tactics-col` passe en grille CSS (`.tactics-layout`) dès 760px de large — échiquier et panneau latéral (thèmes/stats/entraînement personnalisé) côte à côte au lieu d'empiler tout le contenu au-dessus de l'échiquier (cause du bug rapporté : échiquier repoussé sous la ligne de flottaison sur un laptop). L'échiquier est borné par la hauteur de viewport (`dvh`) en plus de la largeur, pour rester entièrement visible sans défilement même sur une fenêtre basse. Mobile inchangé (disposition empilée).

---

### 3.13 undecies EPIC 37 — Migration Chessground (échiquier principal) + coaching visuel

**Fichiers :** `js/board_manager.js`, `js/analysis_feedback.js` (`drawFeedback`), `js/app.js` (`_drawReviewArrows`, `_setModePill`, `_showBoardActive`), `index.html`, `css/style.css`, `assets/js/chessground.min.js` (vendorisé), `assets/css/chessground.base.css` (vendorisé)

> **Périmètre** : seul l'échiquier **principal** (`board_manager.js` — modes Review/Fantôme/Bac à sable) est migré. Les 6 échiquiers de problèmes ad-hoc (`app.js:_createProblemBoard` — Exercice SRS, Coach Tactique, Cimetière des Erreurs, Entraîneur d'Ouvertures, Technique de Mat, Tactical Sprint) restent sur chessboard.js, hors périmètre de cette US.

- **Vendoring (pas de CDN, cohérent EPIC 13)** : `chessground` (npm, GPL-3.0-or-later comme Stockfish déjà vendorisé) rebundlé en IIFE avec esbuild (`--format=iife --global-name=ChessgroundBundle`) — la lib n'est distribuée qu'en ESM par l'éditeur, incompatible avec les scripts classiques (`<script src>` sans `type="module"`) utilisés partout ailleurs dans ce dépôt ; le fichier vendorisé expose directement `window.Chessground`, aucun `<script type="module">` introduit. CSS de base (`chessground.base.css`, mise en page uniquement) vendorisée telle quelle ; les CSS de thème fournies par le paquet (`chessground.brown.css`/`chessground.cburnett.css`) ne sont PAS utilisées — le plateau et les pièces restent pilotés par les mêmes mécanismes ThemeService que chessboard.js (ci-dessous).
- **Aucune validation de coup côté lib** (différence fondamentale avec chessboard.js, qui laissait déposer n'importe où puis rejetait via `onDrop`/`"snapback"`) : Chessground n'autorise que les coups présents dans `movable.dests` (`Map<case_départ, [cases_arrivée...]>`), recalculé à **chaque** position depuis chess.js (`BoardManager._computeDests`, seule source de vérité des règles) — tout coup que l'UI propose est donc légal par construction. `movable.color` (calculé par `_currentMovableColor()`, selon le mode et le trait) verrouille tout déplacement en mode Review, ou hors tour du joueur en Fantôme/Bac à sable.
- **Clic-clic natif** : Chessground gère nativement la sélection + surbrillance des destinations légales (équivalent de `tap_move.js`, EPIC 33) — `board_manager.js` n'attache donc plus `TapMove` sur ce board (toujours utilisé sur les 6 échiquiers de problèmes, inchangés).
- **Thème pièces/plateau (EPIC 18) entièrement piloté par CSS** : contrairement à chessboard.js (`<img>` par pièce, `pieceTheme` résolu en JS), Chessground rend chaque pièce comme `<piece class="{color} {role}">` — `style.css` mappe explicitement chaque combinaison thème/couleur/rôle vers les mêmes SVG (`assets/pieces/{cburnett,cyber-tactics}/`), en réutilisant la classe `body.theme-*` déjà posée par `ThemeService.applySettings` (US 18.2) : **zéro nouveau mécanisme de thème**, `refreshTheme()` n'a donc plus besoin de détruire/recréer le widget (chessboard.js 1.0.0 l'exigeait), un simple `redrawAll()` défensif suffit. Le damier (`cg-board`) est un dégradé CSS pur (technique du damier en `linear-gradient`) sur `--board-square-light/dark` (mêmes variables que chessboard.js) — suit le changement de thème de plateau instantanément.
- **Coaching visuel (`AnalysisFeedback.drawFeedback(cg, playedMove, bestMove)`)** : flèche **rouge** (coup joué/erreur, opacité 0.6) et **verte** (suggestion moteur, opacité 0.8) via `cg.setShapes()` — brushes configurées à la construction du board (`drawable.brushes`, `board_manager.js:_initBoard`). Remplace l'ancien overlay SVG maison (`#board-arrows`, orange/vert, POC v0 §3.13 octies/decies) sur l'échiquier principal uniquement ; les échiquiers de problèmes gardent leur propre mécanisme SVG (`_drawProblemArrows`, inchangé).
- **Surbrillance case sélectionnée + destinations légales** : Chessground ne fournit aucun style par défaut pour `square.selected`/`square.move-dest`/`square.move-dest.oc` (occupée = prise) — `style.css` reprend exactement le même langage visuel (anneau vert = sélection/prise, point vert = coup calme) que `tap_move.js` sur les 6 boards de problèmes, pour une expérience cohérente partout.
- **Vérification visuelle** (capture Playwright, navigateur réel) : damier, pièces (cburnett + cyber-tactics avec lueur néon), flèches rouge/verte, sélection/destinations légales, et bascule d'orientation confirmés au rendu — cf. UserStory.md pour le détail.
...
**Fichiers :** `routers/tactics.py`, `domain/tactical_elo.py`, `domain/tactics.py`, `domain/lichess_puzzles.py` (EPIC 34), `infrastructure/db_client.py`, `infrastructure/pg_repository.py`, `infrastructure/lichess_client.py` (EPIC 34), `supabase/migrations/20260701194517_tactics_epic8.sql`, `supabase/migrations/20260701201530_tactical_attempts.sql`, `js/api_client.js`, `js/app.js`, `index.html` (`#tactics-col`)
...
| Route | Méthode | Comportement |
|---|---|---|
| `/api/v1/tactics/next` | GET | **EPIC 34 :** essaie d'abord l'API Puzzle Lichess (`?angle=` par thème, des millions de puzzles déjà vérifiés) ; repli sur le seed local si Lichess est injoignable, en excluant les derniers problèmes servis à l'utilisateur (variété). Ne renvoie **jamais** `solution`. `?theme_id=` (US 8.2) filtre par catégorie ; absent = « Aléatoire » ; valeur hors `TACTICAL_THEMES` → 422. |
| `/api/v1/tactics/attempt` | POST | Corps `{problem_id, move, time_taken?}` (SAN + secondes optionnelles). Valide le coup **côté serveur** contre la solution stockée (**EPIC 34 :** potentiellement multi-coups — mat en 2, puzzles Lichess), ajuste l'Elo tactique (+15/-15) et la série **seulement à la complétion**, renvoie `{success, complete, new_elo?, solution?, streak?, fen?, opponent_move?}`. `complete:false` = coup juste mais pas final, `opponent_move`/`fen` donnent la réplique adverse auto-jouée à afficher — le joueur continue sur le MÊME `problem_id`. |
...
- **Validation du coup** (`domain/tactics.is_correct_move`) : compare des objets `chess.Move` (python-chess), pas des chaînes brutes — deux annotations différentes d'un même coup légal (ex. `Ra8` vs `Ra8#`) sont reconnues équivalentes. **EPIC 34 :** accepte aussi l'UCI en repli après le SAN (séquences Lichess/mat en 2). Toute notation invalide/illégale sur cette position est un échec silencieux (pas d'exception), jamais une confiance au client sur le résultat.
- **Solutions multi-coups (EPIC 34, `domain/tactics.advance_tactical_attempt`/`solution_sequence`)** : une solution est une séquence de coups (SAN unique historique normalisé en liste à un élément, ou liste explicite — ex. mat en 2 : `[coup_joueur, réplique_forcée, mat]`). La réplique adverse est **auto-jouée** côté serveur (jamais recalculée : ligne déterministe fournie par la donnée) ; l'état de progression (position courante, coups restants) vit en mémoire (`db_client.get_tactical_attempt_session`/`set_.../clear_...`, clé `user_id:problem_id`) entre deux appels `POST /attempt` sur le même problème.
- **Sélection adaptative** (`domain/tactics.select_nearest_problem`) : problème dont `difficulty_elo` est le plus proche de l'Elo tactique du joueur ; tirage aléatoire parmi les problèmes équidistants. **EPIC 34 (`exclude_ids`)** : les derniers problèmes servis à l'utilisateur (`db_client.get_recent_tactical_problem_ids`, 4 derniers, en mémoire) sont écartés du pool avant de chercher le plus proche — corrige le bug « toujours le même exercice » (un pool de 5-6 valeurs bien espacées et un Elo qui bouge peu rendaient un seul problème quasi-systématiquement « le plus proche »). N'exclut jamais la totalité du pool. `db_client.get_next_tactical_problem(tactical_elo, category=None, exclude_ids=None)` filtre par catégorie si fourni.
- **API Puzzle Lichess en source primaire (EPIC 34, `domain/lichess_puzzles.py` + `infrastructure/lichess_client.py`)** : `GET /tactics/next` essaie d'abord Lichess (`angle_for_theme` mappe `mate_in_1`/`mate_in_2`/`hanging_piece` → `mateIn1`/`mateIn2`/`hangingPiece` Lichess) — des millions de puzzles déjà vérifiés, sans les limites d'un petit seed statique. `parse_puzzle_payload` traduit la réponse : `game.pgn` rejoué via python-chess jusqu'à `puzzle.initialPly + 1` demi-coups (**hotfix prod 04/07** : `initialPly` est l'index 0-based du *dernier coup déjà joué*, pas le nombre de coups à rejouer — une confusion documentée uniquement détectée sur une vraie réponse en production, cf. UserStory.md) ; `puzzle.solution` est conservée **intégralement** comme séquence à résoudre (aucun élément auto-joué). Toute panne (réseau, timeout, forme inattendue) est loggée (payload brut tronqué inclus, pour tout futur écart) puis déclenche un repli **immédiat** sur le seed local — même politique que Chess.com (§4.2). Les puzzles récupérés sont insérés dans le même registre que le seed (`db_client.add_lichess_tactical_problem`, cache borné à 200 entrées en FIFO).
...
- **Seed de données (MVP, repli local)** : 15 problèmes (5 `mate_in_1`, 4 `hanging_piece`, 6 `mate_in_2`, Elo 650-1400), **chacun vérifié programmatiquement par python-chess** avant intégration (coup légal + `board.is_checkmate()` pour les catégories mat, capture d'une pièce sans défenseur pour `hanging_piece`). **Bug EPIC 34 corrigé** : les 5 `mate_in_2` n'enregistraient que le 1ᵉʳ coup (un simple échec) comme solution unique, alors qu'un mat immédiat existe aussi sur la colonne a (`Qa4#` etc.) — ce « mat en 1 » plus rapide était injustement rejeté. `solution` est désormais la séquence complète `[coup1, réplique_forcée, mat]` (UCI), chaque ligne re-vérifiée (réplique noire **unique**, mat effectif — `tests/test_db_tactics.py:_is_forced_mate_in_2`). Le même jeu est répliqué en dur dans `db_client._TACTICAL_PROBLEMS_SEED` pour le mode in-memory dev/test et dans la migration SQL pour Supabase — désormais un simple **repli** derrière l'API Puzzle Lichess (EPIC 34) plutôt que la seule source.
...
- Feedback visuel : halo vert/rouge autour de l'échiquier (`.tactics-board--success`/`--error`), message de résultat sous l'échiquier. **EPIC 32 :** plus d'enchaînement automatique — un bouton « Problème suivant → » explicite attend que l'utilisateur soit prêt ; l'échec ne révèle plus la solution en texte mais par des flèches sur l'échiquier (coup joué en orange, coup attendu en vert). **EPIC 34 :** sur un coup juste mais pas final (`complete:false`), la réplique adverse est auto-jouée sur l'échiquier (`this._tacticsChess.move(result.opponent_move)`) et le joueur enchaîne sur le MÊME problème, sans notification d'échec/Elo prématurée ; `_tacticsPlyFen` retient la position de l'étape en cours pour que la flèche de solution (en cas d'échec en cours de route) parte du bon endroit. Son : omis (marqué optionnel dans le DoD).
- **Garde anti-course (EPIC 34)** : `_showTactics()` charge un problème « Aléatoire » dès l'ouverture, avant même qu'un thème soit choisi — `_loadTacticalProblem` porte un jeton de requête monotone (`_tacticsLoadToken`) pour qu'une réponse devenue obsolète (une requête plus récente a démarré entre-temps, ex. clic rapide sur un thème) ne soit jamais appliquée au DOM/état.
...
### 4.8 bis EPIC 37 — Moteur de Puzzles : catalogue Lichess local (US 37.1)

**Fichiers :** `domain/puzzles_models.py`, `domain/lichess_puzzles.py` (`resolve_random_puzzles`), `infrastructure/pg_repository.py` (`count_puzzles`/`get_random_puzzles`/`get_puzzle_filter_sql`), `routers/tactics.py` (`GET /tactics/random`), `scripts/ingest_lichess_puzzles.py`, `supabase/migrations/20260706000000_puzzles_service.sql`, `tests/test_puzzles_service.py`, `tests/test_ingest_lichess_puzzles.py`

> **Distinct de l'EPIC 8/34** (`GET /tactics/next`, un seul problème à la fois, source = seed local *ou* appel réseau live à l'API Puzzle Lichess). L'EPIC 37 ingère au contraire le **dump public complet** Lichess (des millions de puzzles) dans une table Postgres locale, interrogeable par lot (`?limit=`) sans dépendance réseau à l'exécution — les deux systèmes coexistent, `/tactics/random` ne remplace pas encore `/tactics/next` côté frontend (cf. §9).

**Route** (JWT requis) :

| Route | Méthode | Comportement |
|---|---|---|
| `/api/v1/tactics/random` | GET | `?rating_min=&rating_max=&theme=&limit=` — puzzles tirés du catalogue local `lichess_puzzles` dans la plage d'Elo demandée. `rating_max < rating_min` ou `theme` hors `LichessTheme` → 422. Base non configurée (`DATABASE_URL` absent) → 503. Aucun puzzle (même après fallback) → 404. |

**Règles métier :** voir §5.25.
...
**Frontend (US 38.2 — adapté au nouveau contrat)** : carte **OUVERTURES** → vue plein écran `#openings-trainer-col` (`body.openings-trainer-active`).
- **Import** : formulaire nom + PGN (`textarea#ot-pgn`, variations entre parenthèses acceptées — `parse_pgn_tree` les gère). La grille d'ouvertures populaires (EPIC 27, inchangée) pré-remplit le PGN en un clic (séquence SAN sans numéros de coup — `parse_pgn_tree` l'accepte).
- **Session de pratique** : `app.js:_loadNextOpeningMove` interroge `GET /next-move`, affiche rang + score de maîtrise, échiquier indépendant (même fabrique `_createProblemBoard` que l'Entraîneur de Finales, US 8.3) orienté selon le trait de la position servie. `_onOtDrop` capture le SAN joué et le soumet à `POST /attempt` — validation 100 % serveur.
- **Feedback (mêmes conventions que les autres modules de problèmes, EPIC 32)** : halo vert/rouge, flèches de solution (`_showProblemSolution`) sur échec (`solution` révélée par le backend uniquement après coup), bouton « Position suivante → » explicite (pas d'enchaînement automatique), XP/Streak sur succès.
- **Session terminée** : `{"session_complete": true}` → message gamifié (« 🎉 Rien à travailler pour l'instant »), ex. quand le seul nœud débloqué vient d'échouer (repart en révision à J+1, aucun nœud `learning` restant).
- Vérifié en navigateur (Playwright + Chromium, backend/frontend locaux) : import PGN → échiquier de pratique monté → coup faux (halo rouge, flèches, session terminée après clic « suivant », cohérent avec le cooldown d'1 jour) → nouvel import → coup juste (halo vert, maîtrise 0→15, XP crédité). Aucune erreur console.
- `frontend/tests/e2e/openings.spec.js` (EPIC 9) reste retiré (cf. §6.3) — un remplacement e2e pour le nouveau flux n'a pas été ajouté dans cette US (couvert par la vérification manuelle ci-dessus + les tests Jest `api_client.test.js`).
...
### 5.25 Sélection aléatoire par plage d'Elo + fallback ±100 (EPIC 37)

```
count = COUNT(*) WHERE rating BETWEEN rating_min AND rating_max [AND themes @> ARRAY[theme]]
si count == 0 :
    rating_min, rating_max = max(0, rating_min − 100), rating_max + 100   # fallback
    count = COUNT(*) avec la plage élargie
si count == 0 : aucun puzzle (404 côté route)
offset = random.randint(0, count − min(limit, count))     # tiré en Python, PAS en SQL
puzzles = SELECT ... WHERE ... ORDER BY puzzle_id LIMIT min(limit, count) OFFSET offset
```
...
Tactique (EPIC 8/34). La solution (`move_san` du nœud) n'est incluse dans la
réponse `POST /attempt` **qu'en cas d'échec** (`solution: null` sur un
succès) — jamais révélée par avance via `GET /next-move`.
...
| `analysis_feedback.test.js` (EPIC 22/25/37) | AnalysisFeedback | `createState` (états indépendants), `shouldDispatch` (1er dispatch, 20 ré-émissions identiques bloquées, raffinement cpLoss autorisé, arrondi infra-centipion, bascule book→moteur, coups indépendants, entrées invalides), `shouldAlert` (une seule alerte par coup blunder/mistake, classifications non alertables, reset par nouvelle partie), `evalForPlayer`/`exerciseQuality` (EPIC 25), `isExerciseMoveCorrect` (EPIC 26 : SAN strict, préfixe UCI, entrées invalides), `describeMoveFr`/`explainMoveFr` (EPIC 31), **`drawFeedback` (EPIC 37 : flèches rouge+verte, une seule, aucune, entrée malformée, `cg` absent/incomplet — double `{setShapes: jest.fn()}`, jamais de vraie API Chessground)** — **43 TUs, 100 % lignes/branches/fonctions** |
...
| `tap_move.test.js` (EPIC 33, câblage pointerdown/pointerup EPIC 36) | TapMove | `decide` (les 5 issues — select/reselect/move/clear/ignore — avec toutes les combinaisons `isOwnPickable`/`isLegalTarget`, options par défaut), `clearHighlights`/`highlightMoves` (dot vs anneau selon capture, résilience si `chess.moves` lève), `attach` (sélection → surbrillance → coup joué/refusé, non-duplication d'écouteur au second `attach`, `getChess` indisponible, vrai glisser > seuil ignoré car déjà géré par chessboard.js, `pointerup` orphelin sans `pointerdown` préalable) — avec un faux conteneur/case/document minimal (pas de jsdom dans ce dépôt, cf. note ci-dessous) — **22 TUs** |
| `board_manager.test.js` (audit mutation testing 07/2026 ; EPIC 37) | BoardManager | Construction (Worker WASM + **Chessground** mockés, brushes rouge 0.6/vert 0.8 configurées), `_currentMovableColor`/`_computeDests`/`_syncBoard` (EPIC 37 : verrouillage Review, autorisation Sandbox au tour du joueur, `Map` de destinations légales depuis chess.js), **régression bug `isGameOver`** (modes Sandbox/Ghost ne plantent plus, `_onCgMove`/`_sandboxPlayEngineMove` utilisent `chess.game_over()`, la vraie API de chess.js 0.10.3 vendorisé), mode Review (`startReview`, navigation `goToMove`/`nextMove`/`prevMove`, bornes), mode Ghost (`startGhost`, réplique adverse auto-jouée, coup illégal sans exception), utilitaires (`flipBoard`, `reset`, `refreshTheme`, `destroy`), messages du worker Stockfish (`ready`/`info`/`bestmove`) — **23 TUs, chess.js vendorisé réel (pas de mock), Worker/Chessground mockés** |

> `advanced_stats.js` n'est pas dans `collectCoverageFrom` (comme `app.js`, `auth.js`, `board_manager.js`, `tap_move.js`) : seules ses fonctions pures sont testées, les `render*` sont de la glue DOM. `cognitive_dashboard.js` y a été ajouté (EPIC 19, 87 % lignes/branches). `analysis_feedback.js` y a été ajouté (EPIC 22, 100 %). `board_manager.js` reste hors `collectCoverageFrom` malgré ses 23 TUs (18 audit 07/2026 + 5 EPIC 37) : la majorité du fichier est de la glue Worker/DOM (gestion WASM/fallback asm.js, callbacks Chessground/Stockfish) qui ne peut pas atteindre le seuil de 80 % — même précédent que `tap_move.js`.
>
...
- **Backend — mutmut**, module par module sur `app/domain/*.py` (chaque fichier ciblé avec ses seuls tests unitaires, pour éviter de rejouer les 1086 TUs à chaque mutant) :
  ```bash
  cd backend
...
| **Sidebar de navigation + Mes Parties** (EPIC 27) | `index.html:.app-shell` + `app.js:_navigateTo/_setActiveView/_renderGamesLibrary/_triggerManualSync/_clientSideSync/_submitGamesToBackend/_renderOpeningsGrid` | Remplace l'ancien modal PGN (collage manuel supprimé) : sidebar 4 destinations avec deep-link, bibliothèque Mes Parties (sync Chess.com à la demande, table V/L/½), grille visuelle de 10 ouvertures curatées (mini-échiquiers statiques). **Sync client-first** (évolution PO 04/07) : le navigateur — qui détient déjà les parties chargées pour l'affichage — les pousse au backend via `/games/analyze` (pré-filtre client + déduplication hash PGN US 7.2, plafond 5) ; `POST /games/sync` n'est plus qu'un repli quand le navigateur lui-même ne joint pas Chess.com. Piggy-back silencieux au login (`_connectUser`). Courbe d'Elo : même principe (`AdvancedStats.buildEloCurvePoints`, fonction pure testée répliquant les règles backend) si `/stats/elo-curve` répond 502 |
...
| **Coach Tactique — variété, mat en 2 corrigé, Lichess** (EPIC 34) | `domain/lichess_puzzles.py` + `infrastructure/lichess_client.py` + `domain/tactics.py:advance_tactical_attempt/select_nearest_problem(exclude_ids)` + `routers/tactics.py` + `app.js:_submitTacticsAttempt/_loadTacticalProblem` | `GET /tactics/next` essaie l'API Puzzle Lichess en premier (repli sur le seed local, variété par exclusion des derniers problèmes servis) ; les 5 positions `mate_in_2` du seed acceptent désormais la séquence complète (coup, réplique forcée auto-jouée, mat) au lieu d'un simple échec — le mat en 1 alternatif (colonne a) n'est plus injustement rejeté ; garde anti-course (`_tacticsLoadToken`) entre le chargement « Aléatoire » et un thème choisi rapidement |
...
| **Séparation des pages & échiquiers fiabilisés** (EPIC 26) | `index.html:#exercise-col` + `app.js` (Page Exercice SRS, `_createProblemBoard`) + `board_manager.js` (mode exercice retiré) + `analysis_feedback.js:isExerciseMoveCorrect` | Page Exercice plein écran indépendante de la Review (KISS), pill « Exercice » retiré, resize post-layout sur les 6 échiquiers de problèmes (pièces toujours jouables), Échap ferme les modales |
...
- **Table `eco_reference` (EPIC 38)** : peuplée par `scripts/seed_eco.py`, mais **non consommée par une route API** — référentiel brut, prêt pour un futur enrichissement (suggestion de variantes depuis `top_openings`/`successRatio`, cf. §11.9).
...
| 🟢 | Exécuter `seed_eco.py` en production | Table `eco_reference` prête (EPIC 38) mais non consommée par une route API — référentiel brut pour un futur enrichissement (suggestion de variantes depuis `top_openings`/`successRatio`, cf. §11.9) |
| 🟢 | Couverture e2e du nouveau flux Ouvertures | `openings.spec.js` (EPIC 9) retiré, pas remplacé — le flux import/pratique (US 38.2) n'est couvert que par vérification manuelle + Jest `api_client.test.js`, pas par un test bout-en-bout rejouable |

**Historique — déjà fait (chantiers précédemment listés ici comme « à faire ») :**
...
- Adaptation du frontend au Lotus Mastery Engine (EPIC 38, US 38.2, §4.9) : ✅ implémentée et vérifiée visuellement (import PGN, session de pratique, feedback succès/échec, session terminée).
...
Les pistes d'amélioration restantes — toutes **optionnelles et non bloquantes** (pool de connexions Postgres, évals du Stockfish navigateur postées vers `/games/analyze`, import CSV batch de la base Lichess complète, build WASM auto-hébergé, bonus vitesse/leaderboard du Sprint, exécution formelle de mutmut, seuils cognitifs configurables, `DELETE /flashcards/{id}`, message pédagogique à 1 snapshot de progression) — sont consignées au **§11 Backlog**, leur place naturelle : rien ici n'attend d'être « câblé ».

**Dette E2E découverte (EPIC 34, non corrigée — hors périmètre Coach Tactique)** : `cognitive_flashcards.spec.js` et `endgames.spec.js` (§6.3) attendent encore l'ancien texte de solution en clair, remplacé par des flèches depuis l'EPIC 32 — 4 tests en échec, jamais mis à jour à l'époque. À corriger dans une prochaine US Cimetière/Finales.
...
- ~~**Dataset de puzzles externe** (Lichess Puzzle Database) pour élargir le seed tactique~~ — ✅ Traité par EPIC 34 (§4.8, §11.3) : API Puzzle Lichess en source primaire.
...
### 11.3 Puzzles externes — ✅ Traité par EPIC 34, complété par EPIC 37

`GET /tactics/next` interroge l'API Puzzle Lichess en source primaire, par thème, avec repli sur le seed local (§4.8, EPIC 34). **L'import batch du CSV complet évoqué ci-dessous est désormais fait** (§4.8 bis, EPIC 37) : `scripts/ingest_lichess_puzzles.py` + table `lichess_puzzles` + `GET /tactics/random`, avec sélection par plage d'Elo/thème et fallback ±100 sans `ORDER BY random()`. Reste : exécuter l'ETL en production (§9/§10) et brancher une UI dessus (Chessground, §10).
...
```
Branche : claude/chessimprover-puzzles-engine-bd8gt9
